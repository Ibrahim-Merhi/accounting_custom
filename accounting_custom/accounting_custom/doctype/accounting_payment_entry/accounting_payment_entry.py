...
class AccountingPaymentEntry(AccountsController):
	...
	def validate_row(self, row):
		...
		rate = get_company_exchange_rate(self.company, row.currency, self.company_currency, self.posting_date)
	...
	def get_gl_entries(self):
		entries = []
		for row in self.accounts:
			account_currency = frappe.get_cached_value("Account", row.account, "account_currency") or self.company_currency
			if account_currency not in (row.currency, self.company_currency):
			...
			entries.append(
				frappe._dict(
					...
					debit_in_account_currency=flt(row.debit), credit_in_account_currency=flt(row.credit),
