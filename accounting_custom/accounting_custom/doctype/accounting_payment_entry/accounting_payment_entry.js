frappe.ui.form.on("Accounting Payment Entry", {
	...
	company(frm) {
		...
		frm.clear_table("accounting_rows");
	...
	posting_date(frm) {
		(frm.doc.accounting_rows || []).forEach((row) => update_row_rate(frm, row));
	},
});
...
function refresh_payment_row(frm, row) {
	setTimeout(() => {
		const grid_row = frm.fields_dict.accounting_rows.grid.grid_rows_by_docname[row.name];
		if (!grid_row) return;
		grid_row.refresh();
