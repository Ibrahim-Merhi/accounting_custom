...
class CurrencyExchange(Document):
	def validate(self):
		...
		if self.source_account == self.target_account:
			frappe.throw(_("Account From and Account To must be different."))
	...
	def _validate_branch(self):
		branch_company = frappe.db.get_value("Branch", self.custom_branch, "custom_company")
		if branch_company != self.company:
			frappe.throw(_("Branch does not belong to the selected company."))

	def _set_amounts(self):
		from_base_rate = flt(get_company_exchange_rate(
			self.company, self.from_currency, self.company_currency, self.posting_date
		)["exchange_rate"])
		to_base_rate = flt(get_company_exchange_rate(
			self.company, self.to_currency, self.company_currency, self.posting_date
		)["exchange_rate"])
	...
	def on_submit(self):
		if self.journal_entry:
			frappe.throw(_("A Journal Entry is already linked to this Currency Exchange."))
		journal = frappe.get_doc({
			...
			"multi_currency": 1,
			"user_remark": self.remarks or _("Currency Exchange {0}").format(self.name),
		})
	...
	def _base_rate(self, currency):
		return flt(get_company_exchange_rate(
			self.company, currency, self.company_currency, self.posting_date
		)["exchange_rate"])
