const daily_movement_arabic_print_format = `
		...
		<div class="daily-movement-print">
			...
			{% var currency_labels = {LBP: "الليرة اللبنانية", USD: "الدولار الأمريكي"}; %}
			{% var currencies = [...new Set(original_data.filter(row => row.currency).map(row => row.currency))]; %}
			{% for item in currencies %}
				{% var currency_symbol = item === "LBP" ? "ل.ل" : item === "USD" ? "$" : item; %}
				...
				<div class="currency-section">
					<div class="section-title">{{ currency_labels[item] || item }} ({{ item }})</div>
					<div class="summary">
